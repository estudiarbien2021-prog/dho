...
export function MatchDetailModal({
  ...
  const get1x2Percentages = () => {
    ...
    console.log('🔍 1X2 PERCENTAGES CALCULÉS:', {
      homePercent,
      drawPercent,
      awayPercent
    });
  ...
  const getBttsPercentages = () => {
    ...
    console.log('🔍 BTTS PERCENTAGES CALCULÉS:', {
      yesPercent,
      noPercent
    });
  ...
  const getOver25Percentages = () => {
    ...
    console.log('🔍 O/U 2.5 PERCENTAGES CALCULÉS:', {
      overPercent,
      underPercent
    });
  ...
  const secondAIRecommendation = allRecommendations.length > 1 ? {
    ...
    vigorish: opportunities[1]?.type === '1X2' ? match.vig_1x2 : opportunities[1]?.type === 'BTTS' ? match.vig_btts : match.vig_ou_2_5,
    probability: opportunities[1]?.type === '1X2' ? Math.max(match.p_home_fair, match.p_draw_fair, match.p_away_fair) : opportunities[1]?.type === 'BTTS' ? Math.max(match.p_btts_yes_fair, match.p_btts_no_fair) : Math.max(match.p_over_2_5_fair, match.p_under_2_5_fair)
  } : null;
  ...
  return <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 text-sm text-text-weak">
              ...
              {format(new Date(match.kickoff_utc), 'dd/MM/yyyy HH:mm', {
              locale: fr
            })}
            </div>
          </DialogTitle>
        ...
        <div className="grid gap-6">
          ...
          {allRecommendations.length > 0 && <Card className="p-6 bg-gradient-to-r from-primary/5 to-accent/5">
   ...
   {allRecommendations.map((rec, index) => {
                ...
                return <div key={index} className={`p-4 rounded-lg border-l-4 ${isMainConsensus ? 'bg-yellow-50 border-yellow-500' : index === 0 ? 'bg-primary/15 border-primary' : index === 1 ? 'bg-secondary/15 border-secondary' : index === 2 ? 'bg-accent/15 border-accent' : 'bg-muted/15 border-muted'}`}>
                         <div className="flex items-center justify-between">
                           <div className="flex-1">
                             <div className="flex items-center gap-2 mb-1">
                               {isMainConsensus && <span className="text-xl">⭐</span>}
                               <Badge variant={isMainConsensus ? "default" : index === 0 ? "default" : "outline"} className={`text-xs ${isMainConsensus ? 'bg-yellow-500 text-white' : ''}`}>
                                 {isMainConsensus ? 'RECOMMANDATION PRINCIPALE' : `#${index + 1} ${index === 0 ? 'PRIORITÉ MAX' : `PRIORITÉ ${opportunity?.priority || 'N/A'}`}`}
                               </Badge>
                               <span className="font-semibold text-sm">{rec.betType}</span>
                               {rec.detectionCount > 1 && <Badge variant="outline" className="text-xs bg-blue-50 text-blue-700">
                             ...
                             <div className="flex items-center gap-3 mt-2">
                               ...
                               {rec.isInverted && <Badge variant="destructive" className="text-xs">STRATÉGIE INVERSÉE</Badge>}
                             </div>
                           </div>
                         </div>
                         
                         {/* Reasoning for this recommendation */}
                         {rec.reason && rec.reason.length > 0 && <div className="mt-3 p-2 bg-white/50 rounded text-xs text-muted-foreground">
                ...
                <div className="mt-4 p-3 bg-muted/30 rounded-lg">
                  <div className="text-sm text-muted-foreground">
                    ...
                    {allRecommendations.some(r => r.isInverted) && <span className="ml-2 text-orange-600">• Inclut des stratégies contrariennes</span>}
                  </div>
                </div>
              </div>
            </Card>}
          ...
          <Card className="p-4 bg-blue-50">
            ...
            <div className="space-y-2 text-xs text-blue-700">
              ...
              {!preCalculatedRecommendations && opportunities.length > 0 && <div className="mt-2">
                  <strong>Détail des opportunités:</strong>
                  <ul className="list-disc list-inside ml-2">
                    {opportunities.map((opp, i) => <li key={i}>{opp.type}: {opp.prediction} (priorité: {opp.priority})</li>)}
                  </ul>
                </div>}
            </div>
          </Card>
